    ...
    try {
        ...
        try {
            ...
            if ($UserRecord) {
                ...
                $SubAccountRelationships = @(Get-LinkToMeAzDataTableEntity @SubAccountsTable -Filter "PartitionKey eq '$SafeUserId'" | Sort-Object { 
                    if ($_.PSObject.Properties['CreatedAt']) { $_.CreatedAt } else { $_.Timestamp }
                })
                    ...
                    if ($SubAccountLimit -eq 0) {
                        ...
                        foreach ($relationship in $SubAccountRelationships) {
                            ...
                            if ($SubAccountUser) {
                                ...
                                # Add Disabled flag (not AuthDisabled - that's always true for sub-accounts)
                                if (-not $SubAccountUser.PSObject.Properties['Disabled'] -or $SubAccountUser.Disabled -ne $true) {
                                    if (-not $SubAccountUser.PSObject.Properties['Disabled']) {
                                        $SubAccountUser | Add-Member -NotePropertyName 'Disabled' -NotePropertyValue $true -Force
                                    } else {
                                        $SubAccountUser.Disabled = $true
                                    }
                                    $Updated = $true
                                }
                                ...
                                # Always update tier to match parent's new tier
                        ...
                        foreach ($relationship in $AllowedSubAccounts) {
                            ...
                            if ($SubAccountUser) {
                                ...
                                if ($SubAccountUser.PSObject.Properties['Disabled'] -and $SubAccountUser.Disabled -eq $true -and
                                    $SubAccountUser.PSObject.Properties['DisabledReason'] -and 
